...
public class DatabaseHandler extends SQLiteOpenHelper {
	...
	public void onCreate(SQLiteDatabase db) {
		String CREATE_NOTICES_TABLE = "CREATE TABLE " + TABLE_FINANCE + "("
				+ KEY_ID + " INTEGER PRIMARY KEY," + KEY_DATE + " TEXT," +KEY_EXPENSE+ " INTEGER,"
				+  KEY_CATEGORY + " TEXT," + KEY_DESC
				+ " TEXT," + KEY_INCOME + " INTEGER"+")";
		...
		if (cursor != null) {
			...
			if (cursor.moveToFirst()) {
				do {
					...
					//fobj.setDate(cursor.getString(0));
				//	fobj.setCategory(cursor.getString(0));
		//			fobj.setDescription(cursor.getString(2));
          //          fobj.setExpense(cursor.getInt(3));
					// Adding contact to list
	...
	public List<Finance> getmonthRecords(String month) {
		List<Finance> recordList = new ArrayList<Finance>();
		String selectQuery = "SELECT date,sum(expense),category,description FROM budget group by category";
		SQLiteDatabase db = this.getWritableDatabase();
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					...
					System.out.println("in db="+date);
					String date_split[] = date.split("/");
System.out.println("dateeee in db="+date_split[0]);
					// check the month
					if (date_split[1].equals(month)) {
						Finance fobj = new Finance();
//System.out.println("in equal"+cursor.getString(2)+ " "+cursor.getString(3));
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				do {
					...
					fobj.setCategory(cursor.getString(1));
					System.out.println("ddd "+cursor.getInt(0)+ " "+cursor.getString(1));
					recordList.add(fobj);
	...
	public int getBal() {
		...
		Cursor cursor = db.rawQuery(selectQuery, null);
		if (cursor != null) {
			// get last record's balance
			// this is the updated current balance
			if (cursor.moveToNext()) {
				System.out.println("in="+cursor.getInt(0)+" exp="+cursor.getInt(1));
	...
	//	addTrans(new Finance("",0,"","",getBal()+amount));
	}
